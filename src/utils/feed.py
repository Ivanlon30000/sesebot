...
def feed_all_interactive(db:Redis, chatid:int, group:str) -> Generator[Dict[str, Any] | int, None, None]:
...
def feed_all(db:Redis, chatid:int, group:str) -> Generator[Dict[str, Any], None, None]:
...
def random_feed_interactive(db:Redis, chatid:int, group:str="illust") -> Generator[Dict[str, Any] | bool, None, None]:
    imglist = query_all_illusts_id(db, chatid, group)
    if len(imglist) > 0:
        yield True
...
def random_feed(db, chatid:int, group:str="illust") -> Optional[Dict[str, Any]]:
